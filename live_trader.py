...
from config.settings import (
    PAIR, EXCHANGE, Z_SCORE_WINDOW, ENTRY_Z_SCORE, 
...
class StrategyManager:
    def __init__(self, s1, s2, window):
        self.s1 = s1
        self.s2 = s2
        self.window = window
    ...
    def get_historical_prices(self, symbol):
        try:
            log.info(f"Fetching {self.window} days of historical data for {symbol}...")
            response = self.api_session.get_kline(
                category="spot", symbol=f"{symbol}USDT", interval="D", limit=self.window)
            if response['retCode'] == 0 and response['result']['list']:
                data = response['result']['list']
                df = pd.DataFrame(data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'turnover'])
                df['timestamp'] = pd.to_datetime(df['timestamp'].astype(int), unit='ms')
                df.set_index('timestamp', inplace=True)
                return df.iloc[::-1]['close'].astype(float)
            return None
    ...
    def initialize(self):
        log.info("Initializing Strategy Manager...")
    ...
    def update(self, symbol, price):
        new_row = pd.DataFrame({symbol: [price]}, index=[pd.to_datetime('now', utc=True)])
        self.history_df = pd.concat([self.history_df, new_row]).ffill().iloc[1:]
        self.recalculate_metrics()
...
class PortfolioSimulator:
    ...
    def execute_trade(self, trade_type, price_s1, price_s2, hedge_ratio):
        ...
        capital_per_leg = self.capital / 2
        self.quantities['s1'] = capital_per_leg / price_s1
        self.quantities['s2'] = (capital_per_leg / price_s2) * hedge_ratio

        log_msg = f"EXECUTE PAPER TRADE: ENTER {'LONG' if trade_type == 'long' else 'SHORT'} SPREAD. Long {self.quantities['s1']:.4f} {self.s1} / Short {self.quantities['s2']:.4f} {self.s2}"
        log.info(log_msg)
    ...
    def close_position(self, price_s1, price_s2):
        ...
        # Calculate fees for 4 legs (entry and exit)
...
class LiveTrader:
    def __init__(self):
        ...
        self.strategy = StrategyManager(self.s1, self.s2, Z_SCORE_WINDOW)
    ...
    async def handle_update(self, message):
        try:
            ...
            if not all(self.latest_prices.values()): return # Wait for both prices

            # Update strategy with the price of the asset that just ticked
            ...
            elif self.portfolio.position == 1: # Long spread
            ...
            elif self.portfolio.position == -1: # Short spread
                if curr_z <= EXIT_Z_SCORE or curr_z > STOP_LOSS_Z_SCORE:
                    self.portfolio.close_position(self.latest_prices[self.s1], self.latest_prices[self.s2])
    ...
    async def run(self):
        ...
        connector = BybitConnector() # Using Bybit for real-time data
        symbols_to_subscribe = [f"{self.s1}USDT", f"{self.s2}USDT"]
        
        log.info(f"Subscribing to public trade stream for {symbols_to_subscribe}")
        # We use the public trade stream for real-time price updates
